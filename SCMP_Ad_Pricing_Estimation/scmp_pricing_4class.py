...
def analyze_scmp_4class():
    # Load Data
    try:
        df = pd.read_csv('ad_reclassified_4class.csv')
    ...
    plot_data = plot_data[plot_data['Type_Clean'].isin(target_types)]
    ...
    p_labels = {'P1': 'P1\n(Jan-Jun 20)', 'P2': 'P2\n(Jun 20-Jun 21)', 'P3': 'P3\n(Jun 21-Jun 22)'}
    display_map = {t: t.replace('_', ' ').title() for t in target_types}
    ...
    for i, ad_type in enumerate(target_types):
        ...
        if data.empty:
            ...
            ax.set_title(display_map[ad_type])
            continue
        ...
        ax.set_title(f'{display_map[ad_type]} Avg Revenue')
        ax.set_ylabel('HKD')
        for bar in bars:
            if bar.get_height() > 0:
    ...
    plt.tight_layout()
    plt.savefig('scmp_4class_revenue.png', dpi=300, bbox_inches='tight')
    plt.show()
