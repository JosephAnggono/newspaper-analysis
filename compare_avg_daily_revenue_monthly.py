...
def get_fx_rate(year):
    """Returns historical RMB to HKD exchange rate based on year."""
    if year < 2015: return 1.23   # 2011-2014
    elif year < 2019: return 1.20 # 2015-2018
    elif year < 2022: return 1.12 # 2019-2021
    elif year < 2025: return 1.08 # 2022-2024
    else: return 1.07             # 2025+
...
def calculate_revenue(row):
    ...
    if 'bw' in color and 'red' not in color:
        ...
        # If missing, default based on Type
        if 'government' in ad_type or 'other non-commercial' in ad_type:
            color_type = 'bw'
        else:
            color_type = 'color'

    # Size Mapping (AM730 gets Double Full Page for >=150%)
    if newspaper == 'am730':
...
def main():
    ...
    # Parse Date & Get Year for Dynamic FX
    ...
    monthly_rev.columns = ['Newspaper_Lower', 'Month', 'Total_Revenue']
    
    daily_counts = df_filtered.groupby(['Newspaper_Lower', 'Month'])['Date'].nunique().reset_index()
    daily_counts.columns = ['Newspaper_Lower', 'Month', 'Active_Days']
    
    monthly_stats = pd.merge(monthly_rev, daily_counts, on=['Newspaper_Lower', 'Month'])
    monthly_stats['Avg_Daily_Revenue'] = monthly_stats['Total_Revenue'] / monthly_stats['Active_Days']
    monthly_stats['Avg_Daily_Revenue'] = monthly_stats['Avg_Daily_Revenue'].fillna(0)

    # Pivot for Plotting
    pivot_avg_daily = monthly_stats.pivot(index='Month', columns='Newspaper_Lower', values='Avg_Daily_Revenue').fillna(0)
    pivot_avg_daily_smooth = pivot_avg_daily.rolling(window=3, min_periods=1).mean()
    pivot_avg_daily_smooth.index = pivot_avg_daily_smooth.index.astype(str)
    ...
    for paper in TARGET_PAPERS:
        if paper in pivot_avg_daily_smooth.columns:
            ax.plot(pivot_avg_daily_smooth.index, pivot_avg_daily_smooth[paper], 
    ...
    ax.set_title('Average Daily Ad Revenue per Month (HKD) - Dynamic FX', 
                 fontsize=16, fontweight='bold', pad=30)
    ax.set_xlabel('Month', fontsize=12)
    ax.set_ylabel('Avg Daily Revenue (HKD)', fontsize=12)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1000:.0f}K'))
    
    tick_positions = list(range(0, len(pivot_avg_daily_smooth.index), 6))
    tick_labels = [pivot_avg_daily_smooth.index[i] for i in tick_positions if i < len(pivot_avg_daily_smooth.index)]
    ...
    plt.tight_layout()
    plt.savefig('avg_daily_revenue_dynamic_fx.png', dpi=300, bbox_inches='tight')
    plt.show()
    print("✅ Saved: avg_daily_revenue_dynamic_fx.png")
